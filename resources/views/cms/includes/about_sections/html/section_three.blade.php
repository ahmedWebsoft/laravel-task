    ...
    <div class="portlet-body">
      ...
      <div class="col-sm-12">
        ...
        <input type="text" name="heading[]" class="sectionThreeTitle form-control" required
          value="{{$about->where('language_id',$default_language->id)->where('type', 9)->first()->heading}}" />
        </div>
      </div>
      ...
      <div class="col-sm-12">
        ...
        <div class="custom-summernote">
          <textarea name="text[]" id="text" class="sectionThreeDescription summernote" required>
                        {{$about->where('language_id',$default_language->id)->where('type', 9)->first()->text}}
                      </textarea>
        </div>
      </div>
      <br>
      <input type="hidden" name="language_id[]" value="{{$default_language->id}}"
        class="sectionThreeLanguage_id">
      @endforeach
      <br>
      <!-- portlet -->
        ...
        <div class="portlet-body">
          ...
          <div class="card">
            ...
            <div class="col-sm-12">
              ...
              <input type="text" name="heading[]" class="sectionThreeTitle form-control" required
                value="{{$about->where('language_id',$language->id)->where('type', 9)->first()->heading}}" />
            </div>

            <div class="col-md-12">
              <label for="">Description</label>
              <div class="custom-summernote">
                <textarea name="text[]" id="text" class="sectionThreeDescription summernote2" required>
                                {{$about->where('language_id',$language->id)->where('type', 9)->first()->text}}
                              </textarea>
