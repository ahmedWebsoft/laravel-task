        ...
        <div class="col-sm-12">
          <label for="field-1" class="heading control-label">Heading</label>
            <input type="text" name="heading[]" class="sectionOneTitle form-control" required
              value= "{{$home->where('language_id',$default_language->id)->where('type', 1)->first()->heading}}" />                        
        </div> 
        ...
        <div class="col-sm-12">
          ...
          <div class="custom-summernote">
            <textarea name="text[]" id="text" class="sectionOneDescription summernote" required>
              {{$home->where('language_id',$default_language->id)->where('type', 1)->first()->text}}
            </textarea>                        
          </div>
        </div>     
      <br>

      <input type="hidden" name="language_id[]" value="{{$default_language->id}}" class="sectionOneLanguage_id">
      @endforeach
      <br>
        <!-- portlet -->
            ...
            <div class="portlet-heading bg-light-theme">
              ...
              <div class="portlet-body">
                ...
                <div class="card">
                  <div class="card-header">
                   ...
                   <div class="col-sm-12">
                    ...
                    <div class="form-group">
                      <input type="text" name="heading[]" class="sectionOneTitle form-control" required
                        value= "{{$home->where('language_id',$language->id)->where('type', 1)->first()->heading}}" />                        
                    </div>
                  </div>  
                  ...
                  <div class="custom-summernote">
                    <textarea name="text[]" id="text" class="sectionOneDescription summernote" required>
                      {{$home->where('language_id',$language->id)->where('type', 1)->first()->text}}
                    </textarea>
                  <input type="hidden" name="language_id[]" value="{{$language->id}}" class="sectionOneLanguage_id">
                  </div>
                </div>
              </div>
                @endforeach
